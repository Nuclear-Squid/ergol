...
- [Le [E]{.kbd} et la « touche magique » sous un pouce]
- [Les auto-fills]
- [Les combos étendus]
...
fonctionnalités des claviers mécaniques programmables au sein de la disposition.
À l’heure actuelle, aucune de ces fonctionnalités n’est interdite : si c’est
*techniquement faisable* et que ça fait progresser l’ergonomie ou
l’optimisation de la disposition, alors on y a droit.

La disposition est implémentée grâce à un pilote Kanata/QMK prévu pour être
utilisé sur un ordinateur en Ergo‑L.

Pour accélérer le développement de la dispo, nous utilisons un [brute-forceur]
custom, qui ne cherche pas à concevoir la meilleure disposition possible sur la
...
un annulaire, pas plus de 0,2 % de SFU sur un auriculaire…). On reste sur la
méthode d’optimisation classique des Ergonautes, mais ça accélère le travail de
recherche (surtout avec les features d’hypergol qui rendent la conception encore
plus compliquée).
...
Chez les Ergonautes, nous sommes convaincus des apports de la philosophie 1DFH
en termes de confort et d’ergonomie. Cependant, il reste des touches assez
inconfortables dans le pavé de 3×10 central, comme les touches [Z]{.kbd},
...
<!-- Bien que le clavier présenté utilise 4 touches de pouces, on cherche aussi à -->
<!-- prévoir une disposition utilisable sur un clavier avec seulement 3 touches de -->
<!-- pouces (comme avec Arsenik). -->

Retirer 4 touches sur le clavier nous force à trouver de nouvelles places
...
Puisque nous avons prévu la disposition pour des claviers programmables,
on peut se permettre de mettre des lettres sous les pouces. Le `e` étant la
lettre la plus fréquente en français et en anglais, c’est donc celle-ci que nous
...
Passer une lettre sous un pouce permet donc de récupérer une place dans le pavé
de 3×10 pour le `é`, et puisque nous n’avons plus besoin de mettre le tiret sous
...
magique. Cela permet d’avoir un score de SFU très bas (estimé à 0,3 %
...
Bien que la dispo propose une configuration de pouces précise, vous êtes
...
(suivant le nombre de touches de pouces) pour les actions en `hold`. En `tap`,
nous recommandons de placer la touche magique sur le pouce opposé au `e` (car il
est fréquemment doublé en anglais) et sur le même pouce que l’espace (pour
éviter les SFB `e` -> `espace`). Idéalement, l’espace devrait être accessible
avec les deux pouces pour pouvoir faire un enchaînement `magic` -> `espace` sans
avoir un SFB ou un SKB.

Si vous avez 4 touches sous les pouces, une bonne option sur la nouvelle touche
est de rajouter un *one-time shift* (l’équivalent en touche morte d’un *shift*),
...
fiables pour totalement éliminer les SKB et SFK des pouces (qu’on a remarqués
...
Avoir espace et *one-time shift* d’un côté et une lettre (non doublée) et
*magic* de l’autre permettrait d’éliminer ces soucis de configuration. Nous
travaillons sur cette question, mais la solution peut prendre encore du temps.
...
Une façon de comprendre le concept de cette feature est de regarder le
...
Les auto-fills sont une façon beaucoup plus fiable d’implémenter une touche
`Qu`.

Une pression sur la touche `q` va immédiatement écrire un `q`, mais on va
surveiller la touche suivante. Si la touche suivante fait partie de `aeio’é`,
alors on rajoute un `u` juste avant d’écrire la lettre voulue (et si la touche
ne fait pas partie de la liste, alors on continue comme si de rien était). On
peut donc écrire `Que` ou `cinq` sans problème. Ça reste plus compliqué
...
`abc`. Cela permet non seulement d’économiser une touche mais aussi de limiter
...
ne profite pas encore pleinement des possibilités que cela ouvre.

Les « combos étendus »
--------------------------------------------------------------------------------

Les « combos étendus » sont une autre feature qui est exclusive à Hypergol, et
...
seule lettre, on rentre dans une machine à états, qui va écrire le reste du
mot et potentiellement continuer la séquence.

Par exemple, appuyer sur [S]{.kbd} et [F]{.kbd} (de Qwerty) en même temps va
écrire `the`, et donne accès à ces extensions suivantes :
...
avons pas encore ajouté d’autres : il reste des parties plus critiques à régler
avant ça.

« C’est où qu’on signe ? »
--------------------------------------------------------------------------------

Certains d’entre vous vont probablement beaucoup aimer le concept, et
voudront utiliser la disposition le plus tôt possible. Cependant, comme cela a
été mentionné plusieurs fois déjà, c’est encore un travail en cours : il y a des
bugs dans les pilotes, notre analyseur ne sait pas gérer ce type de
dispositions, il manque dans beaucoup d’applis ou frameworks pour
claviers programmables des features qu’on doit implémenter nous-mêmes, il faut
travailler sur le [brute-forceur] custom pour qu’il puisse repérer les mauvaises
redirections (et proposer des auto-fills). La disposition peut encore
radicalement bouger et nous ne sommes pas à l’abri de remettre en cause une
feature existante, ou de découvrir une nouvelle feature.

Pour le dire autrement, nous ne vous recommandons **PAS** Hypergol pour
l’instant, car :

- la disposition n’est pas stable ;
- la disposition est très compliquée à apprendre ;
- les pilotes ne fonctionnent pas ;
- les pilotes sont compliqués à installer ;
...
vous voulez faire partie des *alpha testers*, hésitez pas à venir sur le
[serveur Discord d’Ergo‑L], où on développe la disposition. On serait ravis
